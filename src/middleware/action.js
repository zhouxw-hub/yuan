...
(function () {
     ...
     */
    async function findByPk(database, options, modelName, callback) {
        //验证提交数据
        ...
        //定义模型
        const object = await model[modelName](database);
     ...
     * @param {function} callback 回调方法
     */
    async function destroyByPk(database, options, modelName, callback) {
        //验证提交数据
        var result = verify.execute(options, {
            id: verify.STRING
        });
     ...
     * @returns 存在
     */
    async function existByPk(database, primaryKey, modelName) {
        const model = require("../model/main")(modelName);
        //定义模型
        const object = await model[modelName](database);
     ...
     */
    async function query(database, sql, callback) {
        const sequelize = await Sequelize.initDB(database);
